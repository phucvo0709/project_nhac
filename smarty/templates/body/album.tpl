        ...
        <div class="wrap-body container">
            <div class="row">
                        ...
                        <div class="row">
                            <div class="bar-nhac-trai col-12 col-sm-6 col-md-12 col-lg-12">
                                <div class="content">
                                    ...
                                    <img class="albumImg" src="{$hien_thi->artworkPath}" alt="album">
                                    </span>
                                    <div class="item-right">
                                        ...
                                        <span class="dangBoi">
                                            Đăng bởi: <a href="thanh_vien.php?id={$hien_thi->dangBoi}"><span> {$thanhVien->ten}</span></a>
                                        </span><br>
                                        {if isset($smarty.session.phanQuyen)}
                                            {if $smarty.session.dangBoi == $dangBoi}
                                                <a href="them.php" class="fa fa-plus"></a>
                                                <a href="sua_album.php?id={$hien_thi->id}&caSi={$hien_thi->caSi}&theLoai={$hien_thi->theLoai}" class="fa fa-pencil-square-o"></a>
                                            {/if}
                                            {if $smarty.session.phanQuyen =='1'}
                                                <a href="xoa_album.php?id={$hien_thi->id}" class="fa fa-trash"></a>
                                            {/if}
                                        {/if}
                                    </div>
                                    <div class="trackInfoBar">
                                        <span class="trackNameBar">
                                            <a href="#"><span>{$hien_thi->tenAlbum}</span></a>
                                        </span>
                                        <span class="artistNameBar">
                                            <a href="casi.php?id={$hien_thi->caSi}">{$hien_thi->tenCaSi}</a>
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="item-danh-sach">
                                ...
                                {foreach $doc_bai_hat as $dbh}
                                    <div class="col-12 col-sm-8 col-md-10 col-lg-10">
                                        <h3 class="title-item" onclick='setTrack("{$dbh->id}",{$jsonArray},true)'>
                                                <span>{$dbh->tenBaiHat}</span>
                                        </h3>
                                    </div>
                                    <div class="ca-si-danh-sach col-sm-4 col-md-2 col-lg-2">
                                        <p class="singer-name">
                                               <i class="fa fa-headphones fa-1">{$dbh->luotNghe}</i>
                                               {if isset($smarty.session.phanQuyen)}
                                                    {if $smarty.session.phanQuyen =='1'}
                                                        <a href="album.php?delete={$dbh->id}&id={$hien_thi->id}" class="fa fa-trash"></a>
                                                    {/if}
                                                {/if}
                                        </p>
                                    </div>
                ...
                {foreach $lien_quan_the_loai as $lqtl}
                    <li class="w3-bar">
                        <img src="{$lqtl->artworkPath}" class="w3-bar-item w3-circle w3-hide-small" style="width:86px">
                    <div class="w3-bar-item">
                        <a class="item_bai_hat" href="playlist/{$lqtl->id}/{thu_vien::Bo_dau($lqtl->tenAlbum)}.htm">{$lqtl->tenAlbum}</a><br>
